...
const details = await client.marketplace.tradinghours.details({ 'filter[market]': 'US' });
const status = await client.marketplace.tradinghours.status({ 'filter[market]': 'US' });
...
const bestWorst = await client.marketplace.illio.bestAndWorst('NDX');
const betaBands = await client.marketplace.illio.betaBands('SnP500');
const volume = await client.marketplace.illio.volume('SnP500');
const catPerf = await client.marketplace.illio.categoryPerformance('SnP500');
const catRisk = await client.marketplace.illio.categoryRisk('SnP500');
```
...
// Reports (PDF)
const report = await client.marketplace.praams.reportEquityByTicker('AAPL.US');
```
...
```typescript
const analysis = await client.marketplace.robexia.basicTechAnalysis({ symbol: 'AAPL.US' });
```

### Main Street Data

```typescript
const metrics = await client.marketplace.mainstreetdata.metrics({ symbol: 'TSLA' });
```
