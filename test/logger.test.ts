...
import { createConsoleLogger, NO_OP_LOGGER, redactUrl, resolveLogger } from "../src/logger.js";
...
describe("redactUrl", () => {
  it("replaces api_token value with ***", () => {
    expect(redactUrl("https://eodhd.com/api/eod/AAPL.US?api_token=secret123&fmt=json")).toBe(
  ...
  it("handles api_token at end of query string", () => {
    expect(redactUrl("https://eodhd.com/api/eod?fmt=json&api_token=mykey")).toBe(
      "https://eodhd.com/api/eod?fmt=json&api_token=***",
    );
  });
});
