...
describe('Marketplace: illio', () => {
  ...
  const methods: [string, string][] = [
    ['categoryPerformance', '/mp/illio/categories/performance/SnP500'],
    ['categoryRisk', '/mp/illio/categories/risk/SnP500'],
    ['performance', '/mp/illio/chapters/performance/SnP500'],
    ['bestAndWorst', '/mp/illio/chapters/best-and-worst/SnP500'],
    ['volatility', '/mp/illio/chapters/volatility/SnP500'],
    ['risk', '/mp/illio/chapters/risk/SnP500'],
    ['volume', '/mp/illio/chapters/volume/SnP500'],
    ['betaBands', '/mp/illio/chapters/beta-bands/SnP500'],
  ];

  for (const [method, expectedPath] of methods) {
    it(`${method}() calls ${expectedPath}`, async () => {
      const client = createClient();
      await (client.marketplace.illio as any)[method]('SnP500');
      expect(getCalledUrl(fetch)).toContain(expectedPath);
    });
  }
});
...
describe('Marketplace: Robexia', () => {
  ...
  it('basicTechAnalysis() calls /mp/robexia/basic-tech-analysis', async () => {
    const client = createClient();
    await client.marketplace.robexia.basicTechAnalysis({ symbol: 'AAPL.US' });
    expect(getCalledUrl(fetch)).toContain('/mp/robexia/basic-tech-analysis');
  });
});

describe('Marketplace: Main Street Data', () => {
  let fetch: ReturnType<typeof vi.fn>;
  ...
  it('metrics() calls /mp/mainstreetdata/msdapi', async () => {
    const client = createClient();
    await client.marketplace.mainstreetdata.metrics({ symbol: 'TSLA' });
    expect(getCalledUrl(fetch)).toContain('/mp/mainstreetdata/msdapi');
  });
});
