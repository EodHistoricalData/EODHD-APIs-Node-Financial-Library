...
describe('retry', () => {
  ...
  it('retries on 500 up to maxRetries', async () => {
    const mockFetch = vi.fn()
      ...
      .mockResolvedValueOnce({ ok: false, status: 500, text: () => Promise.resolve('error'), headers: new Headers() })
      .mockResolvedValueOnce({ ok: true, status: 200, json: () => Promise.resolve([]), text: () => Promise.resolve('[]'), headers: new Headers() });
  ...
  it('does not retry on 400 (non-retryable)', async () => {
    const mockFetch = vi.fn()
      .mockResolvedValue({ ok: false, status: 400, text: () => Promise.resolve('bad request'), headers: new Headers() });
    vi.stubGlobal('fetch', mockFetch);
    const client = new EODHDClient({ apiToken: 'test', maxRetries: 2 });
  ...
  it('respects Retry-After header on 429', async () => {
    ...
    const mockFetch = vi.fn()
      .mockResolvedValueOnce({ ok: false, status: 429, text: () => Promise.resolve('rate limited'), headers: headers429 })
      .mockResolvedValueOnce({ ok: true, status: 200, json: () => Promise.resolve([]), text: () => Promise.resolve('[]'), headers: new Headers() });
    vi.stubGlobal('fetch', mockFetch);
  ...
  it('throws after exhausting retries', async () => {
    const mockFetch = vi.fn()
      .mockResolvedValue({ ok: false, status: 503, text: () => Promise.resolve('unavailable'), headers: new Headers() });
    vi.stubGlobal('fetch', mockFetch);
    const client = new EODHDClient({ apiToken: 'test', maxRetries: 2 });
  ...
  it('does not retry on 401 (auth error)', async () => {
    const mockFetch = vi.fn()
      .mockResolvedValue({ ok: false, status: 401, text: () => Promise.resolve('unauthorized'), headers: new Headers() });
    vi.stubGlobal('fetch', mockFetch);
    const client = new EODHDClient({ apiToken: 'test', maxRetries: 2 });
    await expect(client.eod('AAPL.US')).rejects.toThrow(EODHDError);
    expect(mockFetch).toHaveBeenCalledTimes(1);
...
describe('rate limit headers', () => {
  ...
  it('logs warning when rate limit remaining is low', async () => {
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true, status: 200, json: () => Promise.resolve([]), text: () => Promise.resolve('[]'), headers,
    });
    vi.stubGlobal('fetch', mockFetch);
  ...
  it('handles missing rate limit headers gracefully', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true, status: 200, json: () => Promise.resolve([]), text: () => Promise.resolve('[]'), headers: new Headers(),
    });
    vi.stubGlobal('fetch', mockFetch);
    const logger = { debug: vi.fn(), warn: vi.fn(), error: vi.fn() };
    const client = new EODHDClient({ apiToken: 'test', logger });
...
describe('calculateDelay', () => {
  it('returns a value between 0 and capped exponential', async () => {
