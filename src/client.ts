...
export class EODHDClient {
  ...
  /**
   * Marketplace data providers (Unicorn Bay, Trading Hours, illio, Praams, InvestVerte, Robexia, Main Street Data).
   *
   * @example
   * ```ts
   * const markets = await client.marketplace.tradinghours.markets();
   * const analysis = await client.marketplace.robexia.basicTechAnalysis('AAPL.US');
   * ```
   */
